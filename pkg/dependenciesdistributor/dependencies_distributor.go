...
func getPodSpecFromTemplate(workload *unstructured.Unstructured) (*corev1.Pod, error) {
	...
	case util.ReplicaSetKind:
		...
		return lifted.GetPodFromTemplate(&replicaSetObj.Spec.Template, replicaSetObj, nil)
	case util.DeploymentKind:
		...
		return lifted.GetPodFromTemplate(&deploymentObj.Spec.Template, deploymentObj, nil)
	case util.DaemonSetKind:
		...
		return lifted.GetPodFromTemplate(&daemonSetObj.Spec.Template, daemonSetObj, nil)
	case util.StatefulSetKind:
		...
		return lifted.GetPodFromTemplate(&statefulSetObj.Spec.Template, statefulSetObj, nil)
	case util.JobKind:
		jobObj, err := helper.ConvertToJob(workload)
		if err != nil {
			return nil, fmt.Errorf("failed to convert Job from unstructured object: %v", err)
		}
		return lifted.GetPodFromTemplate(&jobObj.Spec.Template, jobObj, nil)
	}
